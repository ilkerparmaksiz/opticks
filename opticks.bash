...
1. run GEOM that defines the CUR functions as side effect
2. pic # list recent screenshots
3. Define the context path string and start time::

   CUR_open context-path-string-including-GEOM
   CUR_touch 12:00


4. pic           # check screenshots move commands
5. pic | sh      # do the moves
6. cd ~/simoncblyth.github.io   # git manage the files



NB sourcing this does nothing other than defining several bash functions starting CUR
---------------------------------------------------------------------------------------


NB different roles of the two CUR.sh files
--------------------------------------------------

::
    cat ~/.opticks/CUR.sh  ## few line CUR.sh holding context
    # generated by /home/blyth/.opticks/GEOM/CUR.sh CUR_
    export CUR=GEOM/LowerWaterDistributorSeg/AfterShiftDec21


    cat ~/.opticks/GEOM/CUR.sh   ## ~142 line CUR.sh controlling the above
    #!/bin/bash
    ...
...
CUR_update(){
   : generated from opticks.bash CUR_template
   rm -f ~/.opticks/GEOM/CUR.sh
   CUR
}
...
GEOM(){
  ...
  local script_CUR=.opticks/GEOM/CUR.sh
  ...
  if [ -f "$HOME/${script_CUR}" ]; then
      source $HOME/${script_CUR}
  else
      echo $BASH_SOURCE : no script_CUR ${script_CUR}
  fi
...
opticks-setup-generate(){
    ...
    local envset=$(opticks-envset-path)
    mkdir -p $(dirname $envset)
    echo $msg writing $envset
    ...
    opticks-envset-generate- > $envset
    rc=$?
    echo $msg post opticks-envset-generate- rc $rc
    [ ! $rc -eq 0 ] && echo $msg ABORT && return $rc
...
opticks-envset-generate-(){ cat << EOS
#!/bin/bash
# envset.sh generated by $FUNCNAME as sibling to bashrc
#
# envset.sh is convenience top level environment setup script that aims
# to do all the environment setup required for a standalone Opticks install
# in one source call, eg for a release::
#
#      source /cvmfs/opticks.ihep.ac.cn/ok/releases/el9_amd64_gcc11/Opticks-vLatest/envset.sh
#
# Or while testing::
#
#      source $(opticks-envset-path)
#
# This script uses /cvmfs/juno.ihep.ac.cn paths to external setup scripts that
# might not be relevant to you.  In this case do not use this script, instead
# make your own "envset.sh" with paths appropriate to your externals.
#
# NB THIS envset.sh IS NOT INTENDED FOR USE TOGETHER WITH OJ(Opticks+JUNOSW) SCRIPTS
# AS THAT WOULD BE DUPLICITOUS AND CONFUSING
#

HERE=\$(dirname \$(realpath \$BASH_SOURCE))

# for compatibility Opticks using the same externals as JUNOSW
...
opticks-full-externals()
{
    ...
    echo $msg config-ing the foreign : eg clhep xercesc g4
    opticks-foreign
    #opticks-foreign-pc
    rc=$?
    [ $rc -ne 0 ] && return $rc
