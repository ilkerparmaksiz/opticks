...
inline std::string sstr::desc( const std::vector<T>& elem )
{
    ...
    {
        if(i % 4 == 0) ss << "\n" ;
        ss << " " << std::fixed << std::setw(10) << std::setprecision(4) << elem[i]  ;
    }
    ss << "\n" ;
    std::string str = ss.str() ;
    return str ;
...
Canonically the prefix hails from SEvt::getInstancePrefix
...
{
    assert( prefix == '\0' || prefix == 'A' || prefix == 'B' || prefix == 'M' || prefix == 'D' );
    assert( idx >= 0 );
